...
class TransactionsImplTests {
    ...
    void getBalance() {
        ...
        ResponseEntity<ResponseTransactionDto> response = new ResponseEntity<>(dto, HttpStatusCode.valueOf(200));
        when(restTemplate.exchange(anyString(), any(), any(), any(Class.class)))
                .thenReturn(response);
        ResponseTransactionDto responseDto = transactionImpl.getTransactions("accountId");
